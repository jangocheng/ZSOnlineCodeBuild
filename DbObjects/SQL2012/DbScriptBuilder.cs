        ...
        public string GetInParameter(List<ColumnInfo> fieldlist, bool output)
        {
            ...
            foreach (ColumnInfo current in fieldlist)
            {
                ...
                switch (typeName.ToLower())
                {
                    case "decimal":
                    case "numeric":
                        plus.Append("@" + columnName + " " + typeName + "(" + precision + "," + scale + ")");
                        goto Label_0288;
                    ...
                    case "nvarchar":
                        {
                            ...
                            plus.Append("@" + columnName + " " + typeName + "(" + dataTypeLenVal + ")");
                            goto Label_0288;
                        }
                    default:
                ...
                plus.Append("@" + columnName + " " + typeName + "(MAX)");
                Label_0288:
        ...
        public string CreatPROCUpdate()
        {
            ...
            foreach (ColumnInfo current in this.Fieldlist)
            {
                ...
                switch (typeName.ToLower())
                {
                    case "decimal":
                    case "numeric":
                        stringPlus.AppendLine("@" + columnName + " " + typeName + "(" + precision + "," + scale + "),");
                        goto Label_03B5;
                    ...
                    case "binary":
                        {
                            string dataTypeLenVal = CodeCommon.GetDataTypeLenVal(typeName.Trim(), length);
                            stringPlus.AppendLine("@" + columnName + " " + typeName + "(" + dataTypeLenVal + "),");
                            goto Label_03B5;
                        }
                    case "varchar":
                    case "nvarchar":
                    case "varbinary":
                        {
                            string str7 = CodeCommon.GetDataTypeLenVal(typeName.Trim(), length);
                            if (str7.Length <= 0)
                            {
                                break;
                            }
                            stringPlus.AppendLine("@" + columnName + " " + typeName + "(" + str7 + "),");
                            goto Label_03B5;
                        }
                    default:
                ...
                stringPlus.AppendLine("@" + columnName + " " + typeName + "(MAX),");
                Label_03B5:
